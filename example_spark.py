...
df = df.select(F.col('quantidadecompras').alias("Quantidade de compras"), F.col('endereco').alias('Endereço'), F.col('sexo').alias('Sexo'), F.col('nome').alias('Nome'), F.col('datanascimento').alias("Data de Nascimento"), F.col('vip').alias('Vip'))

df = df.select('Quantidade de compras', 'Endereço', 'Sexo', 'Nome', getAge('Data de Nascimento').alias('Idade'), 'Vip')
